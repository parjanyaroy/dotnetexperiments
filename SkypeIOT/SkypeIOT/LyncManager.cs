...
namespace SkypeIOT
{
    class LyncManager
    {
        ...
        public string isOutOfOfficeCheck;
        private string _uri;
        ...
        public LyncManager(string email)
        {
            try
            {
        ...
        private void BeginSearchCallback(IAsyncResult r)
        {
            ...
            try
            {
                ...
                else if (results.AllResults.Count == 1)
                {
                    ...
                    timeElapsed = GetElapsedIdleTime(contact);
                    userStatus = availability.ToString();
                    statusCode = contact.GetContactInformation(ContactInformationType.Availability).ToString();
                    isOutOfOfficeCheck = contact.GetContactInformation(ContactInformationType.IsOutOfOffice).ToString();
